...
export async function handleDv12(request:Request,assets:AssetFetcher):Promise<Response>{
  ...
  try{
    ...
    let replayInput=plan?.kind==='followup' && previous ? previous : prepared.execution.request.fastPath ? prepared.execution.request.clauses[0].text : prepared.execution.request.original;
