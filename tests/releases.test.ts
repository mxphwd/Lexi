...
test('release graph uses the audited capability calibration and AD1 remains a badge',()=>{
  ...
  assert.deepEqual(LEXI_RELEASES.map((release)=>release.shortLabel),[
    'Initial build','DV2','DV3','DV4','DV5','DV6','DV7','DV8','DV9','DV10','DV11','DV12','DV13'
  ]);
  ...
  assert.deepEqual(LEXI_RELEASES.map((release)=>release.capabilityIndex),[3,7,12,18,18,27,42,52,57,60,68,76,79]);
  ...
  assert.match(LEXI_RELEASES.at(-1)?.notes.join(' ')??'',/DV13 maintenance/i);
